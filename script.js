...
const pew = new Audio(
  "https://cdn.glitch.global/dbe0dd5a-594b-4eea-8f44-fca4b4042a12/Pew.wav?v=1731723008354"
);
...
medalimage1true.src =
  "assets/medal_1_true.png";

const medalimage2true = new Image();
medalimage2true.src =
  "assets/medal_2_true.png";

const medalimage3true = new Image();
medalimage3true.src =
  "assets/medal_3_true.png";

const medalimage1false = new Image();
medalimage1false.src =
  "assets/medal_1_false.png";

const medalimage2false = new Image();
medalimage2false.src =
  "assets/medal_2_false.png";

const medalimage3false = new Image();
medalimage3false.src =
  "assets/medal_3_false.png";
...
enemyimage5.src =
  "https://cdn.glitch.global/dbe0dd5a-594b-4eea-8f44-fca4b4042a12/beam.png?v=1730187290507";
...
enemyBullet2.src =
  "assets/enemyBullet2.png";

const enemyBeam1 = new Image();
enemyBeam1.src =
  "https://cdn.glitch.global/dbe0dd5a-594b-4eea-8f44-fca4b4042a12/blaster.png?v=1730188061470";
...
function 敵描画() {
  if (!(敵.length === 0)) {
    for (let index = 0; index < 敵.length; index++) {
      ...
      ctx.drawImage(
        enemyimageArray[敵[index].種類].image,
        -敵[index].width / 2,
        -敵[index].height / 2
      );
      ctx.beginPath();
      ...
      if (敵[index].フレーム <= 敵[index].自機追尾フレーム数) {
        if (敵[index].種類 === 3 || 敵[index].種類 === 6) {
          敵[index].variX =
            ((playerX - 敵[index].x) /
              距離を測る(playerX - 敵[index].x, 敵[index].y - playerY)) *
            敵[index].speed;
          敵[index].variY =
            ((playerY - 敵[index].y) /
              距離を測る(playerX - 敵[index].x, 敵[index].y - playerY)) *
          ...
          敵[index].variX =
            敵[index].speed * Math.cos(((敵[index].角度 - 90) * Math.PI) / 180);
          敵[index].variY =
            敵[index].speed * Math.sin(((敵[index].角度 - 90) * Math.PI) / 180);
        }
      }
...
function ボタン生成() {
  ...
  for (let d = 0; d < ボタン.length; d++) {
    ...
    ctx.fillText(
      ボタン[d].value,
      ボタン[d].x + ボタン[d].width / 2,
      ボタン[d].y + ボタン[d].height / 2
    );
  }
}
  ...
  if (1 <= ステージ番号 && ステージ番号 <= 6) {
    ...
    for (let ru = 0; ru < メダル[ステージ番号 - 1].length; ru++) {
      ...
      if (メダル[ステージ番号 - 1][ru].boolean === true) {
        ctx.drawImage(
          メダル[ステージ番号 - 1][ru].imageTrue,
          メダル[ステージ番号 - 1][ru].x,
          550
        );
      } else {
        ctx.drawImage(
          メダル[ステージ番号 - 1][ru].imageFalse,
          メダル[ステージ番号 - 1][ru].x,
          550
        );
      }
  ...
  if (isNumber(str) === true) {
    if (str.length === 12) {
      ...
      if (boolean判定 === true) {
        ...
        グレード =
          Number(str.substr(0, 1)) * 100 +
          Number(str.substr(1, 1)) * 10 +
        ...
        コイン =
          Number(str.substr(9, 1)) * 100 +
          Number(str.substr(10, 1)) * 10 +
          Number(str.substr(11, 1));
      } else {
        アラート処理();
      }
    } else {
